...
- **Rounded square shape:** `$radius-base` (8px)
- **Default size:** 40×40px
- **Image fit:** object-fit: cover
- **Fallback:** initials on `var(--brand-softer)` background, `var(--fg-brand)` text
...
- 4px padding, `$radius-full`, 2px outline in `var(--border-default)` color
- Alternative: `box-shadow: 0 0 0 2px var(--border-buffer), 0 0 0 4px var(--border-default)`

## Stacked Avatars

- Flex row
- Each avatar: 36×36px, `$radius-full`, 2px border in `var(--border-buffer)` color
- Overlap: −12px negative margin on all except first
- Max display: 4 avatars + counter

### Stacked Counter
- Same size as avatars (36×36px), `$radius-full`
- Background: `var(--dark-strong)`, text: white, 12px, semibold
...
- Name: `var(--heading)`, medium weight, 14px
- Subtitle: 13px, `var(--body)` color
- Used in: sidebar user profile, team member rows, client contacts

## Online Indicator

- 10×10px dot, `$radius-full`
- Background: `var(--success)` (online) / `var(--gray)` (offline)
- Position: absolute, 0px bottom, 0px right
- Border: 2px `var(--border-buffer)`
