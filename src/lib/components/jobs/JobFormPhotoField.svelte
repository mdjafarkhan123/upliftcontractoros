<script lang="ts">
	...
	// A photo mid-upload carries a local blob url + `uploading` until the server responds.
	type EditablePhoto = JobFormFieldMedia & { uploading?: boolean };
	...
	let {
		...
		jobId: string;
		fieldId: string;
		photos?: JobFormFieldMedia[];
		canEdit?: boolean;
		cap?: number;
		onAdded: (item: JobFormFieldMedia) => void;
		onRemoved: (id: string) => void;
	} = $props();

	// Display list = the server-confirmed photos (from props) plus any still uploading. Re-seed only
	// when the confirmed set actually changes, preserving in-flight uploads across the re-seed.
	...
	function onFiles(e: Event) {
		...
		if (files.length > allowed.length) toast.error(`Up to ${cap} photos per field.`);
	...
	async function uploadOne(file: File) {
		...
		try {
			...
			fd.append('file', file);
			fd.append('purpose_tag', 'job_form_photo');
