...
export function summarizeRecurrence(r: JobRecurrence): string {
	...
	if (r.freq === 'week') {
		...
		const dayText =
			days.length === 0 ? '—' : joinList(days.map((d) => WEEKDAY_SHORT[d] ?? ''));
		if (n === 1) {
	...
	const weeks = (r.month_weeks ?? []).slice().sort((a, b) => a - b);
	const wds = (r.month_weekdays ?? []).slice().sort((a, b) => a - b);
	const weekText = joinList(weeks.map((w) => NTH[w - 1] ?? `${w}th`));
	const dayText = joinList(wds.map((d) => WEEKDAY_LONG[d] ?? ''));
	if (!weekText || !dayText) return prefix;
	return `${prefix} on the ${weekText} ${dayText}`;
}
