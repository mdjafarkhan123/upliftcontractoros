...
<div class="space-y-8">
	...
	<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
		<div>
			...
			<p class="mt-1 text-sm text-slate-400">Manage organizations and monitor platform events.</p>
		</div>
		<a
			href="/jafar/orgs/new"
		...
		{:else}
			<div class="overflow-x-auto">
				<table class="w-full text-sm">
					<thead class="bg-slate-950/60 text-[11px] font-semibold uppercase tracking-wider text-slate-400">
						<tr>
	...
	>
		<header class="flex items-center justify-between border-b border-slate-800/80 px-5 py-4">
			...
			{#if data.deadLetters.length > 0}
				<span class="inline-flex items-center rounded-full bg-red-500/15 px-2 py-0.5 text-[11px] font-semibold text-red-300 ring-1 ring-red-500/30">
					{data.deadLetters.length} pending
				</span>
			{/if}
		</header>

		{#if error}
			<div class="border-b border-red-500/30 bg-red-500/10 px-5 py-3 text-sm text-red-300" role="alert">
				{error}
			</div>
		{/if}
					...
					<tbody class="divide-y divide-slate-800/70">
						{#each data.deadLetters as event (event.id)}
							<tr class="hover:bg-slate-800/30 transition-colors">
								...
								<td class="px-5 py-3.5 text-xs text-slate-400 max-w-xs truncate" title={event.last_error ?? ''}>
									{event.last_error ?? '—'}
								</td>
