<script lang="ts">
	...
	let editingChip = $state<{
		id: string;
		field: 'probability' | 'stale_after_days' | 'default_follow_up_days';
	} | null>(null);
	let chipDraft = $state('');
	...
	async function performDelete(moveTo: string | null) {
		...
		try {
			...
			toast.success(
				json.data.moved > 0 ? `Stage deleted, ${json.data.moved} moved` : 'Stage deleted'
			);
		} finally {
			deleting = false;
		}
	...
	const wonOptions = [
		{
			value: 'quote_acceptance',
			label: 'On quote acceptance',
			hint: 'The deal wins the moment the client accepts the quote. (Default)'
		},
		{
			value: 'deposit_paid',
			label: 'On deposit paid',
			hint: 'The deal stays in Quoted until the deposit is paid. Quotes with no deposit still win on acceptance.'
		}
	] as const;
</script>
...
<PageWrapper title="Pipeline Stages" subtitle="Customize your pipeline columns" back="/settings">
	<div class="stages">
		{#if canManage && behaviorLoaded}
			<section class="pipeline-behavior">
				<div>
					<h2 class="pipeline-behavior__title">Pipeline behavior</h2>
					<p class="pipeline-behavior__desc">How deals are won and when quiet leads get flagged.</p>
				</div>

				<!-- Won trigger -->
				<div class="pipeline-behavior__field">
					<span class="pipeline-behavior__label">When is a deal "Won"?</span>
					<div class="pipeline-behavior__radios">
						{#each wonOptions as opt (opt.value)}
							<label class="radio-card" class:radio-card--selected={wonTrigger === opt.value}>
								<input
									type="radio"
				...
				<div class="pipeline-behavior__field">
					<span class="pipeline-behavior__label">Ghost-lead suggestion</span>
					<p class="pipeline-behavior__note">
						Suggest marking an open deal as Lost after it's been quiet this long. It's only a nudge
						— nothing is closed automatically.
					</p>
					<div class="pipeline-behavior__days">
						{#each GHOST_DAY_OPTIONS as days (days)}
							<button
								type="button"
								class="pipeline-behavior__day"
								class:pipeline-behavior__day--active={ghostLeadDays === days}
								disabled={savingBehavior}
		...
		{:else if stages.length === 0}
			<EmptyState
				iconClass="ri-layout-column-line"
				title="No stages yet"
			...
			/>
		{:else}
			<p class="stages__hint">Drag to reorder. Tap the % or day chips to edit inline.</p>
			...
			<ul
				...
				use:lazyDndzone={{
					...
					dragDisabled,
					dropTargetStyle: {},
					dropTargetClasses: ['stage-row--dropzone']
				}}
				onconsider={handleConsider}
				onfinalize={handleFinalize}
			>
				{#each stages as stage (stage.id)}
					<li class="stage-row" animate:flip={{ duration: flipDuration }}>
						<i class="ri-draggable stage-row__grip" aria-hidden="true"></i>
						<span class="stage-row__dot" style:background-color={stage.color} aria-hidden="true"
						></span>

						<div class="stage-row__main">
							<div class="stage-row__name-row">
								<p class="stage-row__name">{stage.name}</p>
								{#if stage.is_default}
									<span class="stage-row__default">
							...
							{#if stage.description}
								<p class="stage-row__desc">{stage.description}</p>
							{/if}

							<div class="stage-row__chips">
						...
						<div class="stage-row__actions">
							...
							<button
								type="button"
								class="stage-row__icon-btn stage-row__icon-btn--danger"
			...
			<button type="button" class="stages__add" onclick={openCreate}>
...
{#snippet chip(stage: Stage, field: 'probability' | 'stale_after_days' | 'default_follow_up_days')}
	...
	{@const label =
		field === 'probability' ? '%' : field === 'stale_after_days' ? 'd stale' : 'd follow-up'}
	{#if active}
		<input
			type="number"
			inputmode="numeric"
			class="stage-chip__input"
		...
		/>
	{:else}
		<button type="button" class="stage-chip" onclick={() => startChip(stage.id, field, value)}>
			<span class="stage-chip__value">{value === null ? '—' : value}</span>
			{label}
		</button>
	{/if}
{/snippet}
	...
	<BottomSheet bind:open={editOpen} title="Edit stage">
		{#if editTarget}
			<div class="sheet-fields">
				<div class="field">
					<span class="field__label field__label--required">Name</span>
					<input class="field__input" bind:value={editName} maxlength={60} />
					{#if editNameError}
						<p class="field__error">{editNameError}</p>
					{/if}
				</div>
				...
				<div class="sheet-fields__foot">
					{#if editTarget.is_default}
						<span class="sheet-fields__default-tag">
							<i class="ri-star-fill" aria-hidden="true"></i>
							Default stage
						</span>
					{:else}
	...
	<BottomSheet bind:open={createOpen} title="Add stage">
		<div class="sheet-fields">
			<div class="field">
				...
				/>
				{#if newNameError}
					<p class="field__error">{newNameError}</p>
				{/if}
			</div>
			...
			<div class="sheet-fields__row">
				<div class="field">
					<span class="field__label">Win probability %</span>
					<input
						class="field__input"
				...
				<div class="field">
					<span class="field__label">Stale after (days)</span>
					<input
						class="field__input"
						bind:value={newStale}
						type="number"
			...
			<div class="field">
				<span class="field__label">Auto follow-up after (days)</span>
				<input
					class="field__input"
			...
			<Button loading={creating} loadingLabel="Adding…" onclick={createStage}>
				<i class="ri-add-line" aria-hidden="true"></i>
				Add stage
			</Button>
		</div>
	...
	>
		<div class="sheet-fields">
			{#each moveDestinations as dest (dest.id)}
				<label class="radio-card" class:radio-card--selected={moveToId === dest.id}>
					<input type="radio" class="radio-card__radio" value={dest.id} bind:group={moveToId} />
					<span class="radio-card__dot" style:background-color={dest.color} aria-hidden="true"
					></span>
					<span class="radio-card__label">{dest.name}</span>
				</label>
			{/each}
			...
			>
				Move {deleteCount} deal{deleteCount === 1 ? '' : 's'} & delete stage
			</Button>
		</div>
	</BottomSheet>
{/if}
	...
	<div class="swatches">
		{#each COLOR_PRESETS as c (c)}
			<button
				...
				class:swatches__swatch--selected={selected.toLowerCase() === c.toLowerCase()}
