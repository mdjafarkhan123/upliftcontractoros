# Design Skill Reality Alignment — Sessions 4–6 (Cards + Inputs + Modals + Badges Done)
...
### Badges (5a–5d) — ✅ All applied

| # | Task | Status |
|---|------|--------|
| 5a | Add brand + dark variants to SCSS + component | ✅ `_badge.scss` lines 40–47; `badge.svelte` lines 27–28 |
| 5b | Add `.badge--lg` size variant | ✅ `_badge.scss` lines 67–71 |
| 5c | Drop from skill — badges stay borderless | ✅ Removed `Border: 1px` from both skill docs |
| 5d | Add icon badges, dismissible badges, dot/notification badges, number badges | ✅ `badge--icon-only` (lines 74–82), `badge__dismiss` (lines 85–106), `.badge-dot` (lines 113–126), `.badge-number` (lines 132–145) |

## In Progress (next session — pick one)
...
### Avatars (6a–6d)
See the decisions table below — `_avatar.scss` needs size system, fallback tokens, bordered avatar, online indicator, stacked counter.

### Alerts (7a–7b)
See the decisions table below — `_ui-primitives.scss` + `alert.svelte` need 5 variants + flex row layout.
...
### BADGES (`badges.md` vs `_badge.scss` + `badge.svelte`) — ✅ Done
| # | Decision | Action | Status |
|---|----------|--------|--------|
| 5a | **Add to app** | Add brand + dark variants to SCSS + component | ✅ `_badge.scss` 40–47, `badge.svelte` 27–28 |
| 5b | **Add to app** | Add `.badge--lg` size variant | ✅ `_badge.scss` 67–71 |
| 5c | **Drop from skill** | Badges stay borderless | ✅ Both skill docs updated |
| 5d | **Add to app** | Add icon badges, dismissible badges, dot/notification badges, number badges | ✅ `_badge.scss` icon-only/__dismiss/badge-dot/badge-number |
...
5. ~~**Badges update** — add brand/dark variants, size, sub-components~~ ✅ Session 6
