...
description: >
  ...
  updates, or composing Bits UI primitives with SCSS and CSS custom properties.
  If you are about to write any Svelte component or page — load
  this skill first, every time. Do not guess Svelte 5 syntax from memory.
---

# Svelte UI Patterns — Contractor Growth OS

> Stack: Svelte 5 Runes ONLY · SvelteKit CSR (`ssr = false`) · Bits UI · SCSS
> Types: All shared types live in `$lib/types/index.ts` (Contact, OrgMember, Org, Message, Job, Quote, Invoice, etc.)
> Repo rules from `AGENTS.md` override any older reference wording that mentions shadcn-svelte or Tailwind.
...
- **Rest props via destructuring** — `let { class: className, ...rest } = $props()` when wrapping native elements or Bits UI components. Never duplicate every HTML attribute explicitly.
...
- **SCSS only** — no Tailwind utility classes, no inline styles for static styling. Use `src/lib/styles/*`, component `<style lang="scss">`, and CSS custom properties.
- **Bits UI styled through data attributes** — import Bits UI primitives when needed and style states through documented data attributes.
...
- **Beautiful design, always** — every UI must be polished and mobile-first. Prefer
  existing shared components and Bits UI primitives. Style with SCSS and the project
  design tokens; never write one-off primitives inline on a page when a shared component
  belongs in `$lib/components/shared`.
