...
org_email_settings → org_usage → internal_activity_log → growth_feed_items → media →
...
`organizations.max_*` columns are quota limits. `org_usage` rows are the usage
counters checked against those limits.
...
- Monthly counters use `period_start_date` equal to the first day of the month.
- Lifetime/current-total counters use sentinel `1900-01-01`.
- `storage_bytes` is a lifetime/current-total metric unless explicitly changed by product rules.

**Mutation rules:**

- Primary key is `(org_id, period_start_date, metric)`.
- `value` must never be negative.
- Increment counters atomically with an upsert or row-level lock.
- Set `last_incremented_at = now()` whenever `value` changes.
- Contractors can SELECT own-org usage rows through RLS, but all increments and corrections use service-role API routes or workers.
...
**Status enum:** `draft | sent | viewed | accepted | declined | expired | changes_requested`
...
- Quote deposit payment tracking lives on `quotes`: `deposit_paid_amount` INTEGER, `deposit_paid_at`, `deposit_stripe_payment_intent_id`, and nullable `deposit_applied_invoice_id`
- `deposit_stripe_payment_intent_id` has a UNIQUE partial index (WHERE NOT NULL) — Stripe deposit webhook idempotency guard
- `deposit_applied_invoice_id` references `invoices.id` with `ON DELETE SET NULL`; API logic must verify the invoice and quote share the same `org_id`
- When a quote deposit succeeds, emit `quote.deposit_paid` inside the same transaction that updates the quote deposit fields
...
├── org_usage               (one-to-many by period + metric)
...
│   │   ├── quote_change_requests (one-to-many)
...
- `quotes.deposit_applied_invoice_id` — deposit may be paid before it is applied to an invoice
