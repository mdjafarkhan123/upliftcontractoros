...
description: >
  ...
  styling components, writing SCSS, updating global design tokens, building page
...
> Stack: Svelte 5 · SvelteKit · Bits UI · SCSS and CSS custom properties
...
> Repo rules from `AGENTS.md` override any older reference wording that mentions shadcn-svelte, Tailwind, or `app.css`.
> Never use raw hex values — always use semantic CSS variable tokens from `src/lib/styles/_variables.scss` or global styles.
> If a new one-off color is truly needed, define it as a CSS variable in the shared style tokens first.
...
| Task                                                         | Read first                            |
| ------------------------------------------------------------ | ------------------------------------- |
| Setting up color tokens, CSS variables, global styles        | `references/color-system.md`          |
