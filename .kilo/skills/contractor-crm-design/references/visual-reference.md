...
| Time Tracker card | second `.card--hero` instance — note the reference uses it as the *second* dark card, which works because it's visually distinct (timer digits, not a stat number) from the first; don't add a third |
...
Secondary reference, almost entirely for *structure* rather than color — this
...
*semantic* tokens (`var(--color-bg-app)`, `.badge--success`, etc.), and
