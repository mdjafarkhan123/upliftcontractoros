# Design Skill Reality Alignment — Session 3 (Divergence Audit)

Completed: all 20 component docs audited against live app.
Foundation docs (colors, typography, shadows, radius, layout, borders, brand, sidebars, buttons) were reconciled in sessions 1-2.
...
| # | Decision | Action |
|---|----------|--------|
| 1a | **Skill correct** — update app | Change `_card.scss`: bg=sunk, add 1px border, shadow=xs |
| 1b | **Live correct** — update skill | Radius stays `$radius-2xl` (28px). Update skill doc. |
| 1c | **Add to app** | Create `.card--featured` (brand bg, white text) |
| 1d | **Add to app** | Create `.card--dark` (dark bg, white text) |
| 1e | **Add to app** | Create `.card--section` (full-width, sunk bg, 24px padding) |
| 1f | **Add to app** | Create `.card--interactive` (hover styles) + `.card--static` (no hover) |
| 1g | **Add to app** | Add bottom border + 16/20 padding to `.card__header` |

### INPUTS (`inputs.md` vs `_field.scss`)
| # | Decision | Action |
|---|----------|--------|
| 2a | **Skill correct** — update app | Input bg → `var(--color-bg-surface-sunk)` |
| 2b | **Add to app** | Add `$shadow-xs` to `.field__input` |
| 2c | **Skill correct** — update app | Placeholder → `var(--color-text-secondary)` |
| 2d | **Skill correct** — update app | Label color → `var(--color-text-primary)` |
| 2e | **Update app** — use hard-coded ring | Replace `var(--shadow-focus)` with `0 0 0 1px var(--color-brand)` |
| 2f | **Add to app** | Add success state (green border) to `.field__input` |
| 2g | **Add to app** | Add `.field__prefix` / `.field__suffix` for currency inputs |
| 2h | **Add to app** | Add global search input pattern (280px, expandable, shortcut badge) |
| 2i | **Skill correct** — update app | Error hint font → 12px |
...
1. **Cards rewrite** — update `_card.scss` to match skill model + add featured/dark/section/interactive/static variants
2. **Inputs update** — update `_field.scss` with skill-correct tokens + add success state + prefix/suffix + search
3. **Modals update** — update `_dialog.scss` overlay/radius/sizes + add destructive variant + header border
