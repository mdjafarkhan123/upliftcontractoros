...
**Mutation policy:**
Generally, no `INSERT` / `UPDATE` / `DELETE` RLS policies exist for contractor JWTs.
All mutations flow through SvelteKit server routes using the service role.
Contractor JWTs primarily have SELECT-only policies.
_Exception:_ `push_subscriptions` allows `DELETE` for own rows to enable client-side
...
Five tables have RLS enabled but NO permissive policy for contractor JWTs.
...
| `notification_delivery_state` | Internal push throttling state — service role only      |
...
increments and corrections must flow through service-role API routes or workers.
