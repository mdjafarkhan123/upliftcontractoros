...
An **Invoice** is *"a request for payment which Service Providers send to their clients after the work is
done"* (schema). A **PaymentRecord** is *"payment records applied to a quote or invoice"* (schema) — money
...
> **Build note:** unlike the quote (which tracks *outstanding deposit*), the invoice tracks a live
...
| `sent_not_due` | *"Awaiting payment but not yet due."* Sent to the client; **due date hasn't passed.** (This is the schema's split of "Awaiting Payment" into a not-yet-due state.) |
...
> **Reconciliation:** the overview file (`jobber-00`) summarized the flow as *Draft → Awaiting Payment → Paid
> (Past Due if overdue)*. The real enum adds **`sent_not_due`** (sent-but-not-yet-due, split out from
...
> back to the job's line item). Cost/markup/margin live on the *quote* and *job*, not on the customer-facing
...
### 5.2 `IncomeAdjustmentType` (what the money movement *is*) — from schema
...
> Note the deliberate split: **`CREDIT_CARD`** = a card charged *outside* Jobber (you recording it), vs
> **`JOBBER_PAYMENTS`** = a card/ACH charged *through* Jobber's processor. Same for ACH: `ACH_BANK_PAYMENT`.
...
- **Automatic payments:** with a client's **saved payment method**, invoices can be **auto-created and
  auto-charged** — and when on, **invoice reminders are *not* created** (nothing to chase). [[automatic payments]]
- **Payouts & capital:** `PayoutRecord` / `Payout` / `PayoutStatus` model deposits to the SP's bank;
  ...
  *contractor*, distinct from Wisetack financing to the *client*.
...
- **Requires Invoicing:** an invoice reminder coming due flips the **job** to *Requires Invoicing* so it
...
*(Introspection does not expand `INPUT_OBJECT` fields, so exact create/edit arguments — line-item shape, due
details, client-view options via `InvoiceClientViewOptionsInput` — aren't enumerable from `JobberJson.md`.)*
**No public `PaymentRecordCreate` / refund / `InvoiceSend` mutation** appears in the sampled schema — payments,
refunds, and the actual *delivery* of an invoice are done via the web app / Client Hub / Jobber Payments, not
...
- **Status parity:** we need Jobber's split of "sent but not due" (`sent_not_due`) vs "awaiting payment" vs
  **`past_due`**, plus a real **`bad_debt`** write-off state that keeps the record but removes it from the
  chase lists. Confirm our invoice status enum covers *sent-not-due*, *past-due*, *bad-debt*, and that
...
- **`CREDIT_CARD` (outside) vs `JOBBER_PAYMENTS` (processed) distinction:** our payment model should record
  *how* money came in (offline card vs processed card vs ACH vs Zelle/Venmo/etc.) — Jobber's 15-value
...
- **Capital vs consumer financing:** two different products — **Jobber Capital** (loan to the *contractor*)
  and **Wisetack** (financing for the *client*). Don't conflate; both are partner integrations, both deferred
