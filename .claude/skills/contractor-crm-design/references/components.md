...
*token* you reach for. Always a token, never a raw hex/px typed inline.
...
.btn {
  ...
  &--sm { height: 32px; padding: 0 $space-4; font-size: $text-body-sm; }
  ...
  &--ghost {
    ...
    &:hover { background: var(--color-bg-surface-sunk); color: var(--color-text-primary); }
  }
...
.badge {
  ...
  &--neutral { background: var(--color-bg-surface-sunk); color: var(--color-text-secondary); }

  // solid variant — used for the blue "Followed" tag, which reads as more
  // committed/permanent than the soft "New" tag
  &--solid {
    color: var(--color-text-on-brand);
    &.badge--info    { background: var(--info-solid); }
    &.badge--success { background: var(--success-solid); }
  }
}
...
.card {
  ...
  &--hero {
    ...
    .card__title { color: var(--color-text-on-brand); }
  }
}
...
**Compact dark rail variant** — a second, *intentionally* always-dark rail
...
.sidebar--compact {
  ...
  .sidebar__nav-item {
    ...
    &--active {
      ...
      color: $slate-0;
      &::before { display: none; } // no left-bar in the compact rail
    }
  }
}
```
...
.pipeline-column {
  ...
  &__title { font-weight: $weight-semibold; color: var(--color-text-primary); }
  ...
  &__stats-value { font: $weight-bold #{$text-heading-lg}/1 $font-display; color: var(--color-text-primary); }
  ...
  &__divider {
    ...
    &--applied     { background: var(--info-solid); }
    &--shortlisted { background: var(--success-solid); }
    &--interview   { background: var(--danger-solid); }
    &--evaluation  { background: var(--warning-solid); }
  }
...
.field {
  ...
  &__label { font-size: $text-body-sm; font-weight: $weight-medium; color: var(--color-text-secondary); }
  ...
  &__input {
    ...
    &[data-disabled] { background: var(--color-bg-surface-sunk); color: var(--color-text-muted); }
  }

  &__hint { font-size: $text-body-sm; color: var(--color-text-muted); }
  &__error { font-size: $text-body-sm; color: var(--danger-text); }
}
```
...
.capsule-bar {
  ...
  &--filled { background: var(--color-brand); }
  &--peak   { background: var(--color-brand-strong); } // the tallest/active day
}
```
