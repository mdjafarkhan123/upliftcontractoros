...
| State | Fill | Pattern |
|---|---|---|
| Completed | `var(--color-brand)` solid | Solid fill |
| In Progress | `var(--color-brand-glow)` solid | Solid, lighter |
| Pending / Planned | `var(--color-border)` | Diagonal stripe pattern |
| Active / Today | `var(--color-brand)` | Taller + tooltip on hover |
...
- Track: `var(--color-border)`, 12px stroke, round cap
- Fill: `var(--color-brand)`, 12px stroke, round cap, animated via `stroke-dashoffset`
...
### Full Donut
- Full 360deg circle
- Track: `var(--color-border-strong)`, 8px stroke
- Fill segments: use status colors (`var(--color-brand)`, `var(--warning-solid)`, `var(--color-border)`)
...
- No raw hex in chart colors — use only `var(--color-brand)`, `var(--color-brand-glow)`, status tokens
