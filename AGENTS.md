...
This file governs how Codex Code works in this repository.
...
1.Create or update skills whenever necessary. If you find that a required skill is missing, use the skill creator tool to build it properly rather than guessing. Always notify the user before creating or updating any skill.
...
1. **Expert Engineer Mindset** Think critically, research when needed, always prioritize performance, avoid overengineering
...
1. Build once, reuse everywhere — never silently duplicate. If a UI element or helper is used in multiple places, it should live in a single shared component or utility (src/lib/components/ or src/lib/).
